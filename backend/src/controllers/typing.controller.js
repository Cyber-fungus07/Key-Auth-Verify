...
//
// // Helper function to call FastAPI ML model
// async function callMLVerification(userId, logs) {
//   try {
//     // Create JWT token for FastAPI
//     const token = jwt.sign(
//       { sub: userId, exp: Math.floor(Date.now() / 1000) + (60 * 60) }, // 1 hour expiry
//       JWT_SECRET
//     );
//
//     const response = await fetch(`${FASTAPI_URL}/api/protected/typing/submit`, {
//       method: 'POST',
//       headers: {
...
//     }
//
//     // Save typing data to database
//     const savedWords = [];
//     for (const log of logs) {
//       const { word, data } = log;
//
//       if (!word || !data || !Array.isArray(data)) {
//         console.log('Skipping invalid log — missing word or data');
//         continue;
//       }
//
//       const keystrokes = data.map(k => ({
//         word,
//         key:      k.key,
//         keydown:  k.keydown,
//         keyup:    k.keyup     ?? undefined,
//         holdTime: k.holdTime  ?? undefined,
//       }));
//
//       // Filter out any keystroke missing required fields
//       const valid = keystrokes.filter(k => k.key && k.keydown != null);
//
//       if (valid.length === 0) {
//         console.log(`Skipping "${word}" — no valid keystrokes`);
//         continue;
//       }
//
//       const typingWord = await TypingWord.create({
//         user: userId,
//         data: valid,
//       });
//
//       savedWords.push(typingWord);
//     }
//
//     // Call ML model for verification
//     let mlResult = null;
//     try {
//       mlResult = await callMLVerification(userId, logs);
//     } catch (mlError) {
//       console.error('ML verification failed, but data was saved:', mlError);
//       // Continue with response even if ML fails
//     }
//
//     // Return combined response
//     return res.status(201).json({
//       message: mlResult?.message || 'Typing data submitted successfully',
...
//
//   } catch (err) {
//     console.error('Error saving typing data:', err);
